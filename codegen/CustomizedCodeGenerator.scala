...
object CustomizedCodeGenerator {
  ...
  val db = MySQLDriver.api.Database.forURL(url, driver = jdbcDriver)
  // filter out desired tables
  //  val included = Seq("COFFEES","SUPPLIERS","COF_INVENTORY")
  val codegen = db.run {
    MySQLDriver.defaultTables.map(_.filter(t => true)).flatMap(MySQLDriver.createModelBuilder(_, false).buildModel)
  }.map { model =>
